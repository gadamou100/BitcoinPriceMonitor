...
namespace BitcoinPriceMonitor.Application.Services
{
    public class PriceSourceServices : IPriceSourceServices
    {
        ...
        public async Task<decimal> GetLatestPriceFromSource(string sourceId, string userId)
        {
            ...
            priceSnapShotValue.CreatorId= userId;

             _unitOfWork.GetRepository<PriceSnapshot>().Insert(priceSnapShotValue);
            var channelSaver = _serviceProvider.GetRequiredService<IUnitOfWorkChannelSaver>();
            await channelSaver.AddUnitOfWorkForSaving(_unitOfWork);
